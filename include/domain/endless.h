 ...
 * running out of time. The run starts with a small time budget; every CHECKPOINT_SPACING
 * units of distance travelled tops the timer back up by CHECKPOINT_BONUS_MS (capped at
 * MAX_TIME_MS), so a run that keeps making distance keeps surviving. The caller (app layer)
...
/* Resets `state` to a run's start: EndlessIdle; time_left_ms = START_TIME_MS; distance = 0;
 * next_checkpoint = CHECKPOINT_SPACING; checkpoints_hit = 0. */
 ...
 *   3. While `state->distance >= next_checkpoint`: add CHECKPOINT_BONUS_MS to time_left_ms
 *      (capped at MAX_TIME_MS), advance next_checkpoint by CHECKPOINT_SPACING, and increment
 *      checkpoints_hit. Looped so several checkpoints crossed in one big jump all count.
